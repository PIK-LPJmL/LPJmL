                       ...
                       )
{
  ...
  if(fabs(frac_sum-1)>accuracy)
  {
    ...
    fail(STAND_FRAC_SUM_ERR,TRUE,FALSE,
        "Sum of stand fractions differs from 1 by %g, lakefrac: %g, fracsum: %g  fracsum2: %g cell (%s) from functtion %s() in line %d.\n",
         (frac_sum-1),lakefrac,frac_sum,frac_sum2,sprintcoord(line,&cell->coord),routine,linenr);
#endif
  }
