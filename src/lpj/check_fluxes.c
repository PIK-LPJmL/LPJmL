...
////// Soil->ch4 must be multiplied by WC/WCH4 as well as cell->balance.aCH4_em cell->balance.aCH4_sink, CH4 sink is defined to be negative
  ...
  if(year>startyear && fabs(balance.carbon)>param.error_limit.stocks.carbon)
  {
    ...
    fail(INVALID_CARBON_BALANCE_ERR,FAIL_ON_BALANCE,FALSE,
         ...
         "                            deforest_emissions: %.2f product_turnover: %.2f timber_harvest.carbon: %.2f c_influx %.2f\n",
...
#else
  totw=(cell->discharge.dmass_lake+cell->discharge.dmass_river+cell->discharge.dmass_gw)/cell->coord.area;                   // dmass_gw not needed! should be accounted for in soil->wa pool taken out from soilwater
#endif
  foreachstand(stand,s,cell->standlist)
  {
