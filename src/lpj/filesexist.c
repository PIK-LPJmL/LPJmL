               ...
               )               /** \return TRUE on error */
{
  ...
  bad+=checkinputdata(&config, &config.kbf_filename,"kbf",NULL,LPJ_FLOAT);
  bad+=checkinputdata(&config, &config.slope_filename,"slope_mean",NULL,LPJ_FLOAT);
  bad+=checkinputdata(&config, &config.slope_min_filename,"slope_min",NULL,LPJ_FLOAT);
  bad+=checkinputdata(&config, &config.slope_max_filename,"slope_max",NULL,LPJ_FLOAT);
  ...
  if(config.withlanduse!=NO_LANDUSE)
  {
    ...
    if(config.sdate_option==PRESCRIBED_SDATE)
      bad+=checkinputfile(&config,&config.sdate_filename,NULL,LPJ_SHORT,2*config.npft[CROP]);
