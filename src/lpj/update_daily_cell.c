  ...
  Real balanceW=0;
    ...
    foreachstand(stand,s,cell->standlist)
    {
      if(stand->type->landusetype!=KILL)
      {
        ...
        fprintf(stderr, "landuse==KILL in update_daily_cell(), stand.C=%.3f, standfrac=%.3f\n", (standstocks(stand).carbon + soilmethane(&stand->soil)*WC/WCH4),stand->frac);
    ...
    if(degCtoK(climate->temp)<0)
    {
      if(degCtoK(climate->temp)<(-0.2)) /* avoid precision errors: only fail if values are more negative than -0.2 */
        fail(INVALID_CLIMATE_ERR,FALSE,FALSE,"Temperature=%g K less than zero for cell %d at day %d",degCtoK(climate->temp),cell_id+config->startgrid,day);
      climate->temp=-273.15;
    }
    if(config->with_radiation)
    {
      if(climate->swdown<0)
        fail(INVALID_CLIMATE_ERR,FALSE,FALSE,"Short wave radiation=%g W/m2 less than zero for cell %d at day %d",climate->swdown,cell_id+config->startgrid,day);
    }
    else
    {
      if(climate->sun<-1e-5 || climate->sun>100)
        fail(INVALID_CLIMATE_ERR,FALSE,FALSE,"Cloudiness=%g%% not in [0,100] for cell %d at day %d",climate->sun,cell_id+config->startgrid,day);
      getoutput(&cell->output,SUN,config)+=climate->sun;
    }
    if(climate->windspeed<0)
      fail(INVALID_CLIMATE_ERR,FALSE,FALSE,"Wind speed=%g less than zero for cell %d at day %d",climate->windspeed,cell_id+config->startgrid,day);
#endif
    ...
    foreachstand(stand,s,cell->standlist)
    {
      ...
      if(stand->type->landusetype!=NATURAL && stand->type->landusetype!=WETLAND)
      {
        if(isrice || stand->type->landusetype==SETASIDE_WETLAND)
        {
          ...
          //getoutput(&cell->output,CH4_RICE_EM,config)+=ebul*stand->frac;
...
//  }
  ...
  balanceW=water_after-water_before-climate->prec+
    ...
    if (fabs(end-start1.carbon-CH4_fluxes+fluxes_out.carbon-fluxes_in.carbon)>0.01)
    {
    ...
    end+=cell->ml.product.fast.nitrogen+cell->ml.product.slow.nitrogen+cell->NO3_lateral+
         cell->balance.estab_storage_grass[0].nitrogen+cell->balance.estab_storage_tree[0].nitrogen+cell->balance.estab_storage_grass[1].nitrogen+cell->balance.estab_storage_tree[1].nitrogen;
    if (fabs(end-start1.nitrogen+fluxes_out.nitrogen-fluxes_in.nitrogen)>0.01)
    {
      fail(INVALID_NITROGEN_BALANCE_ERR,FAIL_ON_BALANCE,FALSE,"Invalid nitrogen balance in %s: day: day: %d    %g start: %g  end: %g flux_estab.nitrogen: %g flux_harvest.nitrogen: %g "
      ...
      foreachstand(stand,s,cell->standlist)
        fprintf(stderr,"update_daily: standfrac: %g standtype: %s s= %d iswetland: %d \n",
                stand->frac, stand->type->name,s,stand->soil.iswetland);
    }
