                     ...
                     ) /* returns allocated cell grid or NULL */
{
  ...
  *count=0;
  for(i=0;i<config->ngridcell;i++)
  {
    ...
    if(readcelldata(celldata,&soilcode,grid+i,i,config))
      return NULL;
    ...
    grid[i].ml.cropfrac_rf=grid[i].ml.cropfrac_ir=grid[i].ml.cropfrac_wl[0]=grid[i].ml.reservoirfrac=grid[i].ml.cropfrac_wl[1]=0;
    ...
    else /* read cell data from restart file */
    {
      ...
      if(!grid[i].skip)
        check_stand_fracs(grid+i,
                          grid[i].lakefrac+grid[i].ml.reservoirfrac,5);
      else
        (*count)++;
    }
    if(!grid[i].skip)
