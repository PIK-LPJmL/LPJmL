...
Bool fprintoutputjson(int i,const Config *config)
{
  FILE *file;
  char *filename;
  char **pftnames;
  ...
  filename=malloc(strlen(config->outputvars[i].filename.name)+strlen(SUFFIX)+1);
  if(filename==NULL)
  {
    printallocerr("filename");
    return TRUE;
  }
  filename=strcat(strcpy(filename,config->outputvars[i].filename.name),SUFFIX);
  /* create metafile */
  ...
  fprintf(file,"  \"nstep\" : %d,\n",min(1,getnyear(config->outnames,config->outputvars[i].id)));
  ...
  fprintf(file,"  \"nyear\" : %d,\n",(config->outputvars[i].id==GRID || config->outputvars[i].id==COUNTRY || config->outputvars[i].id==REGION) ? 1 : config->lastyear-config->outputyear+1);
