                     ...
                     )                     /** \return string vector */
{
  ...
  switch(index)
  {
    ...
    case FPC_BFT:
         for(i=0;i<ngrass;i++)
         {
           pftnames[i]=malloc(strlen(pftpar[i+npft-nbiomass-ngrass].name)+strlen("grassland ")+1);
           strcpy(pftnames[i],"grassland ");
           strcat(pftnames[i],pftpar[i+npft-nbiomass-ngrass].name);
         }
         for(i=0;i<ngrass;i++)
         {
           pftnames[i+ngrass]=malloc(strlen(pftpar[i+npft-nbiomass-ngrass].name)+strlen("biomass tree ")+1);
           strcpy(pftnames[i+ngrass],"biomass tree ");
           strcat(pftnames[i+ngrass],pftpar[i+npft-nbiomass-ngrass].name);
         }
         for(i=0;i<nbiomass;i++)
           pftnames[i+2*ngrass]=strdup(pftpar[i+npft-nbiomass].name);
         for(i=0;i<ngrass;i++)
         {
            pftnames[i+ngrass*2+nbiomass]=malloc(strlen(pftpar[i+npft-nbiomass-ngrass].name)+strlen("grassland irrigated ")+1);
            strcpy(pftnames[i+ngrass*2+nbiomass],"grassland irrigated ");
            strcat(pftnames[i+ngrass*2+nbiomass],pftpar[i+npft-nbiomass-ngrass].name);
         }
         for(i=0;i<ngrass;i++)
         {
            pftnames[i+ngrass*3+nbiomass]=malloc(strlen(pftpar[i+npft-nbiomass-ngrass].name)+strlen("biomass tree irrigated ")+1);
            strcpy(pftnames[i+ngrass*3+nbiomass],"biomass tree irrigated ");
            strcat(pftnames[i+ngrass*3+nbiomass],pftpar[i+npft-nbiomass-ngrass].name);
         }
         for(i=0;i<nbiomass;i++)
         {
            pftnames[i+ngrass*4+nbiomass]=malloc(strlen(pftpar[i+npft-nbiomass].name)+strlen("irrigated ")+1);
            strcpy(pftnames[i+ngrass*4+nbiomass],"irrigated ");
            strcat(pftnames[i+ngrass*4+nbiomass],pftpar[i+npft-nbiomass].name);
         }
         break;
  }
  return pftnames;
