  ...
  if(!cell->skip)
  {
    ...
    if(degCtoK(climate->temp)<0)
    {
      if(degCtoK(climate->temp)<(-0.2)) /* avoid precision errors: only fail if values are more negative than -0.2 */
        fail(INVALID_CLIMATE_ERR,FALSE,"Temperature=%g K less than zero for cell %d at day %d",degCtoK(climate->temp),cell_id+config->startgrid,day);
      climate->temp=-273.15;
    }
    if(config->with_radiation)
    {
      if(climate->swdown<0)
        fail(INVALID_CLIMATE_ERR,FALSE,"Short wave radiation=%g W/m2 less than zero for cell %d at day %d",climate->swdown,cell_id+config->startgrid,day);
    }
    else
    {
      if(climate->sun<-1e-5 || climate->sun>100)
        fail(INVALID_CLIMATE_ERR,FALSE,"Cloudiness=%g%% not in [0,100] for cell %d at day %d",climate->sun,cell_id+config->startgrid,day);
      getoutput(&cell->output,SUN,config)+=climate->sun;
    }
    if(climate->windspeed<0)
      fail(INVALID_CLIMATE_ERR,FALSE,"Wind speed=%g less than zero for cell %d at day %d",climate->windspeed,cell_id+config->startgrid,day);
#endif
