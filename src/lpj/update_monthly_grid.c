  ...
  for(cell=0;cell<config->ngridcell;cell++)
  {
    if(!grid[cell].skip)
    {
      ...
      if(fabs(start.carbon-end.carbon)>param.error_limit.stocks_fcn.carbon)
          fail(INVALID_CARBON_BALANCE_ERR,config->fail_on_balance,FALSE,"Invalid carbon balance in %s at the end: month=%d: error=%g start : %g end : %g ",
      ...
      if(fabs(start.nitrogen-end.nitrogen)>param.error_limit.stocks_fcn.nitrogen)
          fail(INVALID_NITROGEN_BALANCE_ERR,config->fail_on_balance,FALSE,"Invalid nitrogen balance in %s at the end: month=%d: error=%g start : %g end : %g ",
