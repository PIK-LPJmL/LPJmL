                  ...
                  )
{
  ...
  annual_climbuf(&cell->climbuf,cell->balance.aevap+cell->balance.atransp,cell->balance.agpp,ncft,config->crop_phu_option!=OLD_CROP_PHU && (config->sdate_option==NO_FIXED_SDATE || year<=config->sdate_fixyear));
