                 ...
                 )                      /** \return nitrogen uptake (gN/m2/day) */
{
 ...
 // NCplant = (vegn_sum_tree(pft)-(tree->ind.sapwood.nitrogen+tree->ind.heartwood.nitrogen)*pft->nind+pft->bm_inc.nitrogen*(tree->falloc.leaf+tree->falloc.leaf))/(vegc_sum_tree(pft)-(tree->ind.sapwood.carbon+tree->ind.heartwood.carbon)*pft->nind+pft->bm_inc.carbon*(tree->falloc.leaf+tree->falloc.leaf)); /* Plant's mobile nitrogen concentration, Eq.9, Zaehle&Friend 2010 Supplementary */
  ...
  {
    if (n_uptake>*n_plant_demand-(vegn_sum_tree(pft)-tree->turn_nbminc+pft->bm_inc.nitrogen-tree->ind.heartwood.nitrogen*pft->nind))
      n_uptake=*n_plant_demand-(vegn_sum_tree(pft)-tree->turn_nbminc+pft->bm_inc.nitrogen-tree->ind.heartwood.nitrogen*pft->nind);
    if(n_uptake<=0)
      n_uptake=0;
    else
    {
  ...
  {
    data=pft->stand->data;
    autofert_n=*n_plant_demand-(vegn_sum_tree(pft)-tree->turn_nbminc+pft->bm_inc.nitrogen-tree->ind.heartwood.nitrogen*pft->nind);
    n_uptake += autofert_n;
  ...
  {
    n_deficit = *n_plant_demand/(1+pft->par->knstore)-(vegn_sum_tree(pft)-tree->turn_nbminc+pft->bm_inc.nitrogen-tree->ind.heartwood.nitrogen*pft->nind);
    if(n_deficit>0 && pft->npp_bnf>0)
    {
      ...
      pft->npp_bnf=0.0;
    if(*n_plant_demand/(1+pft->par->knstore)>(vegn_sum_tree(pft)-tree->turn_nbminc+pft->bm_inc.nitrogen-tree->ind.heartwood.nitrogen*pft->nind))   /*HERE RECALCULATION OF N-demand TO N-supply*/
    {
      NC_actual=(vegn_sum_tree(pft)-tree->turn_nbminc+pft->bm_inc.nitrogen-tree->ind.heartwood.nitrogen*pft->nind)/(vegc_sum_tree(pft)+pft->bm_inc.carbon-tree->ind.heartwood.carbon*pft->nind);
      NC_leaf=(tree->ind.leaf.nitrogen-tree->turn.leaf.nitrogen+pft->bm_inc.nitrogen*tree->falloc.leaf/pft->nind)/(tree->ind.leaf.carbon-tree->turn.leaf.carbon+pft->bm_inc.carbon*tree->falloc.leaf/pft->nind);
  ...
  case BIOMASS_TREE:
    ...
    getoutputindex(&pft->stand->cell->output,PFT_NDEMAND,nnat+rbtree(ncft)+data->irrigation*nirrig,config)+=max(0,ndemand_all-(vegn_sum_tree(pft)-tree->turn_nbminc+pft->bm_inc.nitrogen-tree->ind.heartwood.nitrogen*pft->nind))/365; /* stand->cell->ml.landfrac[data->irrigation].biomass_tree; */
    break;
  case AGRICULTURE_TREE:
    data=pft->stand->data;
    if(config->pft_output_scaled)
  ...
  case WOODPLANTATION:
    ...
    getoutputindex(&pft->stand->cell->output,PFT_NDEMAND,nnat+rwp(ncft)+data->irrigation*nirrig,config)+=max(0,ndemand_all-(vegn_sum_tree(pft)-tree->turn_nbminc+pft->bm_inc.nitrogen-tree->ind.heartwood.nitrogen*pft->nind))/365; /* stand->cell->ml.landfrac[data->irrigation].biomass_tree; */
    break;
  default:
    if(config->pft_output_scaled)
    ...
    getoutputindex(&pft->stand->cell->output,PFT_NDEMAND,pft->par->id,config)+=max(0,ndemand_all-(vegn_sum_tree(pft)-tree->turn_nbminc+pft->bm_inc.nitrogen-tree->ind.heartwood.nitrogen*pft->nind))/365;
  } /* of 'switch' */
  pft->stand->cell->balance.n_uptake+=n_uptake*pft->stand->frac;
  pft->stand->cell->balance.n_demand+=max(0,(ndemand_all-(vegn_sum_tree(pft)-tree->turn_nbminc+pft->bm_inc.nitrogen-tree->ind.heartwood.nitrogen*pft->nind)))*pft->stand->frac/365;
  return n_uptake;
} /* of 'nuptake_tree' */
