                 ...
                 )                    /** \return total N demand  (gN/m2) */
{
 ...
 if(tree->ind.leaf.carbon==0)
  ...
  printf("\n");
  printf("ndemand_to = %g vegn %g vegc %g ndemand_leaf %g nc_ratio %g %g \n",ndemand_tot,vegn_sum_tree(pft)-tree->turn_nbminc+pft->bm_inc.nitrogen-tree->ind.heartwood.nitrogen*pft->nind,vegc_sum_tree(pft),*ndemand_leaf,nc_ratio,*ndemand_leaf/(tree->ind.leaf.carbon*pft->nind+pft->bm_inc.carbon*tree->falloc.leaf));
#endif
  return ndemand_tot;
} /* of 'ndemand_tree' */
