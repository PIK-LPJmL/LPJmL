                       ...
                       )                      /** \return TRUE on error */
{
  ...
  error(rc);
  rc=nc_def_dim(cdf->ncid,(index==SOILC_LAYER || index==SOILN_LAYER || index==SOILNO3_LAYER || index==SOILNH4_LAYER || index==SOILTEMP || index==SWC) ? "layer" : "npft",size,&pft_dim_id);
  error(rc);
