...
void test_with_drip_irrigation_of_same_temp_soiltemps_should_not_change(void)
{
  ...
  for (int i = 0; i < 10; i++)
  {
    ...
    infil_perc_irr(st, 20, initial_temp * c_water + c_water2ice, &return_flow_b, 1, 1, &test_config);
  }
...
void test_with_sprinkler_irrigation_of_same_temp_soiltemps_should_not_change(void)
{
  Stand *st = setup_simple_stand();
  Real return_flow_b = 0;
  Real initial_temp = 5.0;
