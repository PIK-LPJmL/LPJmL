  ...
  *snowmelt=0.0;
  ...
  if(soil->snowpack>epsilon)
  {
    ...
    *temp_bsnow=temp;
    ...
    //   //dT=th_diff_snow*timestep2sec(1.0,heat_steps)/(c_watertosnow*c_watertosnow*soil->snowpack*soil->snowpack*1e-6)
    //   //   *(temp+(soil->temp[TOPLAYER]*(1-soil->litter.agtop_cover)+soil->litter.agtop_temp*soil->litter.agtop_cover)-2*soil->temp[SNOWLAYER]);
    //   dT=1/6.3e5*timestep2sec(1.0,heat_steps)/(c_watertosnow*c_watertosnow*soil->snowpack*soil->snowpack*1e-6)
    //      *((temp- soil->temp[SNOWLAYER])*0.2 +((soil->temp[TOPLAYER]*(1-soil->litter.agtop_cover)+soil->litter.agtop_temp*soil->litter.agtop_cover)- soil->temp[SNOWLAYER])*1 );
