...
)                     /** \return water runoff (mm) */
{
  ...
  Real water_after, water_before,balancew,gw_start,wa_start;
  ...
  //fprintf(stderr,"start ground_st_am:%g ground_st:%g dmass_gw:%g wa:%g\n",stand->cell->ground_st_am,stand->cell->ground_st,stand->cell->discharge.dmass_gw,soil->wa);
#endif
  ...
  maxit=0;
  while((infil > epsilon || freewater > epsilon) && maxit<250)
  {
    NO3perc_ly=0;
    maxit++;
    freewater=0.0;
...
//        }
        ...
        if ((soil->w[l]*soil->whcs[l])>(soildepth[l]-soil->freeze_depth[l])*(soil->wsat[l]-soil->wpwp[l]))
        {
          ...
          if(enth) reconcile_layer_energy_with_water_shift(soil,l,-grunoff,vol_water_enth,config); /* subtract enth of runoff until influx, assuming vol_enth of above layer  */
          ...
          *return_flow_b+=grunoff*(1-stand->frac_g[l]);
          ...
          else
          {
            ...
            //lrunoff[l]+=grunoff;  //no leaching lower than water table
          }
        }
        ...
        if (soildepth[l]>soil->freeze_depth[l])
        {
          ...
          if((soil->w[l]+soil->ice_depth[l]/soil->whcs[l]-1)>epsilon/soil->whcs[l])
          {
            ...
            if (maxit>249)
              fprintf(stderr,"maxit==250; Cell (%s); prec: %g soil->wtable %3.3f perc %3.3f  runoff: %3.3f lat_runoff: %3.3f lateral_water: %3.3f\n",sprintcoord(line,&stand->cell->coord),prec,soil->wtable,perc,runoff,lat_runoff_last,stand->cell->lateral_water);
#endif
  ...
  for(l=0;l<NSOILLAYER;l++)
  {
    ...
    if(runoff_out<0)
    {
      //fprintf(stderr,"hier3 runoff_out:%g\n", runoff_out);
        if(soil->w_fw[l]>runoff_out)
        {
...
//    }
  ...
  {
    qcharge_tot1=lat_runoff_last+runoff;                                //outflux recharge from the layer above water table
    runoff=0;
  }
  else
    qcharge_tot1=lat_runoff_last;


 /* if(qcharge_tot1>10)
  {
    for(l=(BOTTOMLAYER-1);l==0;l--)
    {
      soil->w[l]+=soil->w_fw[l]/soil->whcs[l];
      soil->w_fw[l]=0;
      run=0;
      fprintf(stderr,"Layer:%d qcharge_tot1: %g soil->w:%g \n", l, qcharge_tot1,soil->w[l]);
      soil->w[l]+=qcharge_tot1/soil->whcs[l];
      if((soil->w[l]*soil->whcs[l] )> (soil->wsats[l]-soil->wpwps[l]))
      {
        run=(soil->w[l]*soil->whcs[l])-(soil->wsats[l]-soil->wpwps[l]);
        soil->w[l]-=run/soil->whcs[l];
         qcharge_tot1=run;
      }
      else
      {
      ...
      fprintf(stderr,"Layer:%d qcharge_tot1: %g soil->w:%g run:%g \n", l, qcharge_tot1,soil->w[l],run);
    }
  }
  */
//  for(l=0;l<NSOILLAYER;l++)
...
//    }
  ...
  {
    if (soil->wtable==layerbound[lwt] || (soil->psi_sat[lwt]+soil->wtable-layerbound[lwt])<epsilon || soil->wtable<0)
    {
      vol_eq = soil->wsat[lwt];
    }
    else if (lwt==0)
    {
      temp0 =pow(((soil->psi_sat[lwt]+soil->wtable-layerbound[lwt])/soil->psi_sat[lwt]),(1-1/soil->b[lwt]));
      tempi = 1;
      voleq1 = -soil->psi_sat[lwt]*soil->wsat[lwt]/(1-1/soil->b[lwt])/(soil->wtable-layerbound[lwt])*(tempi-temp0);
  ...
  zq=-soil->psi_sat[lwt]*pow(tmp_vol,-soil->b[lwt]);        // eq. 7.126 equilibrium soil matric potential
...
/*
  slug=qcharge_tot;
  while(abs(slug) > epsilon)
  {
      qcharge_tot=max(-10,slug);
      qcharge_tot=min(10,slug);
      slug-=qcharge_tot;
*/
  ...
  else
  {
    ...
    if(soil->wtable>=layerbound[BOTTOMLAYER-1])
    {
      if (soil->wtable>0)
        ...
        S=soil->wsat[BOTTOMLAYER-1]*(1.-pow((1.+(1/soil->psi_sat[BOTTOMLAYER-1])),(-1./soil->b[BOTTOMLAYER-1])));
      S=max(S,0.02);
...
//  }  //end while loop
  ...
  for(l=(BOTTOMLAYER-1);l==0;l--)
    if(runoff_out<0)
    {
      if(soil->w[l]>epsilon)
      {
        soil->w[l]+=runoff_out/soil->whcs[l];
        runoff_out=0;
        if(soil->w[l]<0)
        {
          runoff_out=soil->w[l]*soil->whcs[l];
  ...
  forrootsoillayer(l)
   if (soil->w[l]< -epsilon || soil->w_fw[l]< -epsilon )
    {
      fprintf(stderr,"5.2 infil_perc Cell (%s) soilwater=%.6f soilice=%.6f wsats=%.6f\n",
          sprintcoord(line,&stand->cell->coord),allwater(soil,l),allice(soil,l),soil->wsats[l]);
      fflush(stderr);
  ...
  stand->cell->discharge.dmass_gw+=outflux*stand->frac*stand->cell->coord.area;
  stand->cell->ground_st_am+=outflux*stand->frac*0.05;
  stand->cell->ground_st+=outflux*stand->frac*0.95;
  ...
  water_after=soilwater(&stand->soil);
  balancew=water_after-water_before-prec+runoff_surface+runoff_out+drain_perched_out+rsub_top+runoff+((stand->cell->ground_st_am+stand->cell->ground_st)-gw_start)/stand->frac;
  if(fabs(balancew)>0.1 && stand->frac>epsilon)
  {
    fail(INVALID_WATER_BALANCE_ERR,FAIL_ON_BALANCE,FALSE,
        "Invalid water balance in %s: balanceW: %g water_before: %.6f water_after: %.6f standtype: %s standfrac: %g runoff_surface: %g "
        "drain_perched_out: %g runoff_out: %g rsup_top_lastl: %g runoff: %g rsub_top: %g rw_buff: %g wa: %g startwa: %g infil: %g qcharge_tot2: %g  isrice= %d\n",
        __FUNCTION__,balancew,water_before,water_after,stand->type->name,stand->frac,runoff_surface,drain_perched_out,runoff_out,rsup_top_lastl,runoff,rsub_top,soil->rw_buffer,soil->wa,wa_start,prec,qcharge_tot2,isrice);
    fprintf(stderr,"ende ground_st_am:%g ground_st:%g balance_gw:%g standfrac: %g\n",stand->cell->ground_st_am,stand->cell->ground_st,(stand->cell->ground_st_am+stand->cell->ground_st)-gw_start,stand->frac);
  }
#endif
  ...
  {
    //fprintf(stderr,"dmass_gw:%g ground_st_am:%g ground_st:%g outflux:%g  lat_runoff_last:%g  runoff_out:%g  rsup_top_lastl:%g \n",stand->cell->discharge.dmass_gw,stand->cell->ground_st_am,stand->cell->ground_st,outflux,lat_runoff_last,runoff_out,rsup_top_lastl);
