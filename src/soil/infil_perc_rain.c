                    ...
                    )                     /** \return water runoff (mm) */
{
  ...
  jwt=0;
    ...
    {
      for(l=jwt;l>=0;l--){
        if(l==0 && layerbound[l]>soil->wtable){
          layerbound2=-50;
        } else {
          layerbound2=layerbound[l];
        }
    ...
    {                                      // deepening water table (negative qcharge)
      for(l=jwt;l<BOTTOMLAYER;l++){
        if(layerbound[l]>soil->wtable){
          layerbound2=layerbound[l];              // TODO TEST IF REALLY NECESSARY
        } else {
          layerbound2=layerbound[l+1];
        }
        if(soil->wtable!=layerbound2 && l<icet)
        {
          if (soil->wtable>0)
    ...
    {
      if(soil->wtable<frost_depth){                                                //WATER_TABLE ABOVE FROST_DEPTH
        k_perch=0;
        wtsub=0;
        for(l=jwt;l<=icet;l++){
        ...
        if (wtsub > 0) k_perch=k_perch/wtsub;                                       // ks is adjusted as in pedotransfer.c
  ...
  if(soil->wtable>frost_depth)
  {
     ...
     for(l=icet;l<=jwt;l++){
      ...
      if(jwt>=(BOTTOMLAYER-1)) depthsum=layerbound[BOTTOMLAYER-1];
      ...
      if(soil->wtable>=layerbound[BOTTOMLAYER-1]){
        ...
        if(rsub_top_tot > 0){
          if(rsub_top_tot > 1) fprintf(stderr,"RSUB_TOP IS POSITIVE = %g in Drainage!\n",rsub_top_tot);
  ...
  for(l=0;l<NTILLLAYER;l++)
  {
    ...
    /*writing output*/
  stand->cell->output.mseepage+=(outflux+lat_runoff_last+rsub_top)*stand->frac;
  stand->cell->output.mrunoff_lat+=(runoff_out+drain_perched_out)*stand->frac;
