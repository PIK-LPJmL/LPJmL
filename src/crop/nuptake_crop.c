                 ...
                 )                      /** \return nitrogen uptake (gN/m2/day) */
{
  ...
  if (*n_plant_demand > pft->bm_inc.nitrogen)
  {
    ...
    {
      if(with_nitrogen==AUTO_FERTILIZER)
      {
        ...
        crop->nfertsum+=autofert_n*pft->stand->frac;
#else
        pft->stand->cell->output.cft_nfert[pft->par->id-npft+data->irrigation*(ncft+NGRASS+NBIOMASSTYPE)]+=autofert_n;
#endif
