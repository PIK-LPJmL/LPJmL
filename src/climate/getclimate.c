               ...
               )                     /** \return TRUE on error */
{
  ...
  if (isclimate)
 {
  ...
  if(climate->data[0].sun!=NULL)
  {
    ...
    if(config->cloud_filename.fmt==CDF)
      for(i=0;i<climate->file_cloud.n;i++)
        climate->data[0].sun[i]=100-climate->data[0].sun[i];
  }
  ...
  {
    if(readclimate(&climate->file_tamp,climate->data[data_index].tamp,0,climate->file_tamp.scalar,grid,year_climate,1,config))
    {
      if(isroot(*config))
  ...
  {
    if(readclimate(&climate->file_burntarea,climate->data[data_index].burntarea,0,climate->file_burntarea.scalar,grid,year, 1,config))
    {
      if(isroot(*config))
  ...
  else
  {
    ...
    {
      if (isroot(*config))
        fprintf(stderr, "ERROR131: Cannot read temperature anomaly of year %d in getclimate().\n",
          year);
      return TRUE;
    }
    ...
    {
      if (isroot(*config))
        fprintf(stderr, "ERROR131: Cannot read precipitation anomaly of year %d in getclimate().\n",
          year);
      return TRUE;
    }
    ...
    {
      if (isroot(*config))
        fprintf(stderr, "ERROR131: Cannot read lwnet anomaly of year %d in getclimate().\n",
          year);
      return TRUE;
    }
    ...
    {
      if (isroot(*config))
        fprintf(stderr, "ERROR131: Cannot read swdown anomaly of year %d in getclimate().\n",
          year);
      return TRUE;
    }
  }
