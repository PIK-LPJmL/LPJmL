                 ...
                 )                     /** \return TRUE on death */
{
  ...
  stress = 0;
#ifdef CHECK_BALANCE
  start.carbon = standstocks(stand).carbon + soilmethane(&stand->soil)*WC/WCH4-pft->establish.carbon+pft->stand->cell->balance.neg_fluxes.carbon;
...
#ifdef CHECK_BALANCE
  end = standstocks(stand).carbon + soilmethane(&stand->soil)*WC/WCH4-pft->establish.carbon+pft->stand->cell->balance.neg_fluxes.carbon;
