                  ...
                  )                      /** \return nitrogen uptake (gN/m2/day) */
{
  ...
  /* Protect against division by zero when plant carbon is very small */
  if(grass->ind.leaf.carbon + grass->ind.root.carbon > epsilon)
  ...
  else
  {
    ...
    if(*n_plant_demand/(1+pft->par->knstore)>(vegn_sum_grass(pft)+pft->bm_inc.nitrogen))
    {
      ...
      *ndemand_leaf=max(0.0,max(grass->ind.leaf.nitrogen*pft->nind-grass->turn_litt.leaf.nitrogen,*ndemand_leaf));
