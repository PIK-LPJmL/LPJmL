                    ...
                    )                     /** \return establishment flux (gC/m2,gN/m2) */
{
    ...
    findlandusetype(cell->standlist,SETASIDE_WETLAND)!=NOT_FOUND)
 {
  ...
  {
    fail(INVALID_CARBON_BALANCE_ERR,config->fail_on_balance,FALSE, "Invalid carbon balance in %s: day: %d %g start: %g end: %g flux_out.carbon: %g fluxes_in.carbon: %g flux_estab.carbon: %g CH4_fluxes: %g",
  ...
  if(fabs(end-start.nitrogen+fluxes_out.nitrogen-fluxes_in.nitrogen-flux_estab.nitrogen)>param.error_limit.stocks_fcn.nitrogen)
      fail(INVALID_NITROGEN_BALANCE_ERR,config->fail_on_balance,FALSE, "Invalid nitrogen balance in %s: day: %d %g start: %.3f  end: %.3f influx: %g outflux: %g flux_estab.nitrogen: %g",
