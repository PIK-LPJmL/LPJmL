                        ...
                        )                      /** \return stand has to be killed (TRUE/FALSE) */
{
  ...
  */
  ...
  else
  {
    ...
    foreachpft(pft,p,&stand->pftlist)
    {
      getoutputindex(&stand->cell->output,PFT_VEGC,getnnat(npft,config)+rbtree(ncft)+biomass_tree->irrigation.irrigation*nirrig,config)+=vegc_sum(pft);
      getoutputindex(&stand->cell->output,PFT_VEGN,getnnat(npft,config)+rbtree(ncft)+biomass_tree->irrigation.irrigation*nirrig,config)+=vegn_sum(pft)+pft->bm_inc.nitrogen;
