                        ...
                        )                      /** \return stand has to be killed (TRUE/FALSE) */
{
  ...
  */
  foreachpft(pft,p,&stand->pftlist)
  {
    if(istree(pft))
    {
      ...
      if(stand->growing_time>=treepar->rotation && stand->growing_time%treepar->rotation==0)
      {
        ...
        stand->cell->ml.image_data->store_bmtree_yield+=yield*stand->frac/treepar->rotation;
#endif
        stand->cell->balance.biomass_yield+=yield*stand->frac;
        if(config->pft_output_scaled)
          stand->cell->output.pft_harvest[rbtree(ncft)+irrigation->irrigation*(ncft+NGRASS+NBIOMASSTYPE+NWPTYPE)].harvest+=yield*stand->frac;
        else
          stand->cell->output.pft_harvest[rbtree(ncft)+irrigation->irrigation*(ncft+NGRASS+NBIOMASSTYPE+NWPTYPE)].harvest+=yield;
  ...
  stand->cell->output.cftfrac[rbtree(ncft)+irrigation->irrigation*(ncft+NGRASS+NBIOMASSTYPE+NWPTYPE)]+=stand->cell->ml.landfrac[irrigation->irrigation].biomass_tree;
  ...
  if(isdead)
  {
    ...
    if(irrigation->irrigation)
    {
      ...
      if(config->pft_output_scaled)
      {
        stand->cell->output.cft_conv_loss_evap[rbtree(ncft)+irrigation->irrigation*(ncft+NGRASS+NBIOMASSTYPE+NWPTYPE)]-=(irrigation->irrig_stor+irrigation->irrig_amount)*(1/irrigation->ec-1)*irrigation->conv_evap*stand->cell->ml.landfrac[1].biomass_tree;
        stand->cell->output.cft_conv_loss_drain[rbtree(ncft)+irrigation->irrigation*(ncft+NGRASS+NBIOMASSTYPE+NWPTYPE)]-=(irrigation->irrig_stor+irrigation->irrig_amount)*(1/irrigation->ec-1)*(1-irrigation->conv_evap)*stand->cell->ml.landfrac[1].biomass_tree;
      }
      else
      {
        stand->cell->output.cft_conv_loss_evap[rbtree(ncft)+irrigation->irrigation*(ncft+NGRASS+NBIOMASSTYPE+NWPTYPE)]-=(irrigation->irrig_stor+irrigation->irrig_amount)*(1/irrigation->ec-1)*irrigation->conv_evap;
        stand->cell->output.cft_conv_loss_drain[rbtree(ncft)+irrigation->irrigation*(ncft+NGRASS+NBIOMASSTYPE+NWPTYPE)]-=(irrigation->irrig_stor+irrigation->irrig_amount)*(1/irrigation->ec-1)*(1-irrigation->conv_evap);
      }
