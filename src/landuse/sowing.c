             ...
             )                     /** \return establishment flux (gC/m2,g/N/m2) */
{
  ...
  if(fabs(end-start.nitrogen+fluxes_out.nitrogen-fluxes_in.nitrogen-flux_estab.nitrogen)>param.error_limit.stocks_fcn.nitrogen)
     fail(INVALID_NITROGEN_BALANCE_ERR,FAIL_ON_BALANCE,FALSE, "Invalid nitrogen balance in %s in cell(%s): day: %d error: %g start: %.3f  end: %.3f influx: %g outflux: %g flux_estab.nitrogen: %g",
