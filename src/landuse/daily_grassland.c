 ...
 */
  ...
  if(isphen)
  {
    ...
    if(config->pft_output_scaled)
    {
      output->pft_harvest[rothers(ncft)+index].harvest+=harvest.harvest*stand->cell->ml.landfrac[data->irrigation.irrigation].grass[0];
      output->pft_harvest[rmgrass(ncft)+index].harvest+=harvest.harvest*stand->cell->ml.landfrac[data->irrigation.irrigation].grass[1];
      output->pft_harvest[rothers(ncft)+index].residual+=harvest.residual*stand->cell->ml.landfrac[data->irrigation.irrigation].grass[0];
      output->pft_harvest[rmgrass(ncft)+index].residual+=harvest.residual*stand->cell->ml.landfrac[data->irrigation.irrigation].grass[1];
    }
    else
    {
      output->pft_harvest[rothers(ncft)+index].harvest+=harvest.harvest;
      output->pft_harvest[rmgrass(ncft)+index].harvest+=harvest.harvest;
      output->pft_harvest[rothers(ncft)+index].residual+=harvest.residual;
      output->pft_harvest[rmgrass(ncft)+index].residual+=harvest.residual;
    }
    /* harvested area */
