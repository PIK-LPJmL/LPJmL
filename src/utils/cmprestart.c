...
int main(int argc,char **argv)
{
  Bstruct file1,file2;
  Bstruct_data data1;
  int level=1,ilevel=1,size,count=0,iarg,i,npft1,npft2;
  double d;
  ...
  else
  {
    ...
    else
    {
      ...
      if(version1!=NULL)
      {
        ...
        if(version2!=NULL)
        {
          if(strcmp(version1,version2))
            fprintf(stderr,"Version %s im '%s' differs from version %s in '%s'.\n",
      ...
      if(!bstruct_readint(file1,"year",&npft1))
      {
        ...
        {
          if(npft1!=npft2)
            fprintf(stderr,"Year %d im '%s' differs from %d in '%s'.\n",
      ...
      if(!bstruct_readint(file1,"npft",&npft1))
      {
        ...
        {
          if(npft1!=npft2)
            fprintf(stderr,"Number of CFTs %d im '%s' differs from %d in '%s'.\n",
                    npft1,argv[iarg],npft2,argv[iarg+1]);
        }
      }
      ...
      if(!bstruct_readint(file1,"datatype",&npft1))
      {
        if(!bstruct_readint(file2,"datatype",&npft2))
        {
          if(npft1!=npft2)
          {
            fprintf(stderr,"Datatype %d im '%s' differs from %d in '%s'.\n",
                    npft1,argv[iarg],npft2,argv[iarg+1]);
            bstruct_finish(file1);
      ...
      case BSTRUCT_BEGINARRAY: case BSTRUCT_BEGINARRAY1:
        ...
        if(level==ilevel)
        {
          if(data1.name==NULL || bstruct_isdefined(file2,data1.name))
          {
            ...
            {
              if(data1.name==NULL)
                fprintf(stderr,"Size of unnamed array=%d in '%s' is different from %d in '%s'.\n",
                        data1.size,argv[iarg],size,argv[iarg+1]);
              else
                fprintf(stderr,"Size of array '%s'=%d in '%s' is different from %d in '%s'.\n",
  ...
  if(count)
    printf("%d differences in values found.\n",count);
  else
