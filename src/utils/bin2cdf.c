                       ...
                       Bool isnetcdf4)
{
  ...
  rc=nc_def_dim(cdf->ncid,netcdf_config->bnds_name,2,&bnds_dim_id);
  error(rc);
  if(!notime)
  {
    ...
    rc=nc_def_var(cdf->ncid,netcdf_config->time_bnds.name,NC_DOUBLE,2,dimids,&time_bnds_var_id);
    error(rc);
  ...
  dimids[0]=lat_dim_id;
  dimids[1]=bnds_dim_id;
  rc=nc_def_var(cdf->ncid,netcdf_config->lat_bnds.name,NC_DOUBLE,2,dimids,&lat_bnds_var_id);
  error(rc);
  rc=nc_def_var(cdf->ncid,netcdf_config->lon.name,NC_DOUBLE,1,&lon_dim_id,&lon_var_id);
  error(rc);
  dimids[0]=lon_dim_id;
  dimids[1]=bnds_dim_id;
  rc=nc_def_var(cdf->ncid,netcdf_config->lon_bnds.name,NC_DOUBLE,2,dimids,&lon_bnds_var_id);
  error(rc);
  if(!notime)
  {
    switch(header.nstep)
    ...
    put_att_text(cdf->ncid, time_var_id,"comment",netcdf_config->time.comment);
    ...
    put_att_text(cdf->ncid, time_bnds_var_id,"comment",netcdf_config->time_bnds.comment);
  ...
  if(map!=NULL)
  {
    ...
    if(map->isfloat)
    {
      ...
      put_att_text(cdf->ncid,bnds_var_id,"standard_name",netcdf_config->depth_bnds.standard_name);
      put_att_text(cdf->ncid,bnds_var_id,"long_name",netcdf_config->depth_bnds.long_name);
      put_att_text(cdf->ncid,bnds_var_id,"comment",netcdf_config->depth_bnds.comment);
    }
    else
    {
      if(isnetcdf4)
      ...
      if(getmapsize(map)==header.nbands)
      {
        ...
        put_att_text(cdf->ncid,pft_var_id,"comment",netcdf_config->pft.comment);
      }
    ...
    for(j=0;j<header.nstep;j++)
      for(k=0;k<header.nbands;k++)
      {
        if(isshort)
        {
          ...
          if(write_short_cdf(cdf,data_short,(notime) ? NO_TIME : i*header.nstep+j,ngrid,ispft,k,netcdf_config.missing_value.s))
          {
            free(data_short);
            fclose(file);
            close_cdf(cdf);
        ...
        else
        {
          ...
          if(write_float_cdf(cdf,data,(notime) ? NO_TIME : i*header.nstep+j,ngrid,ispft,k,netcdf_config.missing_value.f))
          {
            free(data);
            fclose(file);
            close_cdf(cdf);
